  ...
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        ...
        <el-form-item>
          <el-button type="success" v-permit="'manage:resource:add'" @click="add">新增</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    <!--列表-->
    <el-table :data="rows" highlight-current-row v-loading="loading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column prop="controller" label="控制器" sortable></el-table-column>
      <el-table-column prop="action" label="动作" sortable></el-table-column>
      ...
      <el-table-column label="操作" width="126">
        <template slot-scope="scope">
          <el-button type="text" v-permit="'manage:resource:'" size="small" @click="show(scope.$id, scope.row)">查看</el-button>
          <el-button type="text" v-permit="'manage:resource:modify'" size="small" @click="edit(scope.$id, scope.row)">编辑</el-button>
          <el-button type="text" v-permit="'manage:resource:add'" size="small" @click="sublevel(scope.$id, scope.row)">子级</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--底部工具条-->
    <el-col :span="24" class="toolbar">
      <el-button type="danger" v-permit="'manage:resource:delete'" @click="batchRemove" :disabled="this.sels.length===0">删除所选</el-button>
    ...
    <el-dialog :title="form.id ? ('修改[' + form.id + ']') : '新增'" :visible.sync="formVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="80px" :rules="rules" ref="form">
        ...
        <el-form-item label="模块">
          <el-input v-model="form.module" auto-complete="on"></el-input>
        </el-form-item>
        <el-form-item label="控制器">
          <el-input v-model="form.controller" auto-complete="on"></el-input>
        </el-form-item>
        <el-form-item label="动作">
          <el-input v-model="form.action" auto-complete="on"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    search () {
      this.loading = true
      wrapper.tips(resourceService.list(this.filters)).then((response) => {
    ...
    batchRemove: function () {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {type: 'warning'}).then(() => {
        this.loading = true
        wrapper.tips(resourceService.delete(ids), true).then((response) => {
          if (response.code === 0) {
            this.search()
          } else {
            this.loading = false
    ...
    submit () {
      this.$refs.form.validate((valid) => {
        ...
        wrapper.tips(resourceService.save(this.form)).then(response => {
          if (response.code === 0) {
    ...
    edit (id, row) {
      ...
      if (!this.config.ready) {
        ...
        wrapper.tips(resourceService.config()).then((response) => {
          if (response.code === 0) {
